...
function timeSpanToString(startDate, endDate) {
  ...
  return `${addZero(hrs)}:${addZero(min)}:${addZero(sec)}.${addTwoZero(ms)}`;
}
...
function angleBetweenClockHands(date) {
  const hrs = date.getUTCHours() > 12 ? date.getUTCHours() % 12 : date.getUTCHours();
  const min = date.getUTCMinutes();
  const minAngle = min * 6;
  const hrsAngle = hrs * 30 + minAngle / 12;
  const resAngle = Math.abs(hrsAngle - minAngle) * (Math.PI / 180);

  return resAngle > Math.PI ? resAngle - Math.PI : resAngle;
}
